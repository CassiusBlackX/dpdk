...
// 封装为zlib member
static int append_zlib_wrapped_chunk(uint8_t *dst, size_t dst_cap,
                                     size_t *dst_off, uint8_t *raw_deflate,
                                     uint32_t raw_len, uint32_t adler) {
  const uint8_t zlib_header[2] = {0x78, 0x9c};
  size_t needed = sizeof(zlib_header) + raw_len + 4;
  if (*dst_off + needed > dst_cap)
    return -ENOSPC;
...
static size_t dpdk_compress_stateless(
    uint8_t *input, size_t input_len, uint8_t *out_raw, size_t out_raw_cap,
    size_t *raw_sizes, uint8_t *out_zlib, size_t out_zlib_cap,
    size_t *zlib_sizes, size_t *zlib_len, struct rte_mempool *src_pool,
    struct rte_mempool *dst_pool, struct rte_mempool *op_pool, uint16_t dev_id,
    void *priv_xform) {
  size_t input_off = 0, raw_off = 0, zlib_off = 0, chunk_idx = 0;
  while (input_off < input_len) {
    uint32_t this_chunk = (uint32_t)((input_len - input_off) > CHUNK_SIZE_BYTES
                                         ? CHUNK_SIZE_BYTES
                                         : (input_len - input_off));
    ...
    uint8_t *src_data =
        (uint8_t *)(uintptr_t)rte_pktmbuf_append(src, this_chunk);
    uint8_t *dst_data =
    ...
    if (out_zlib && zlib_sizes) {
      ...
      if (append_zlib_wrapped_chunk(out_zlib, out_zlib_cap, &zlib_off, dst_data,
                                    op->produced, adler) != 0) {
...
static size_t dpdk_decompress_stateless(
    const uint8_t *raw_stream, size_t raw_len, const size_t *raw_sizes,
    size_t member_count, uint8_t *out, size_t out_cap,
...
int main(int argc, char **argv) {
  ...
  // 预分配所有缓冲区
  size_t comp_cap = INPUT_SIZE_BYTES + (INPUT_SIZE_BYTES / 8) +
                    (INPUT_SIZE_BYTES / CHUNK_SIZE_BYTES) * 16 + 4096;
  ...
  struct rte_mempool *src_pool = rte_pktmbuf_pool_create(
      "bigc_src_mbuf_pool", MBUF_POOL_SIZE, MEMPOOL_CACHE_SIZE, 0,
      SRC_MBUF_DATA_ROOM + RTE_PKTMBUF_HEADROOM, rte_socket_id());
  struct rte_mempool *dst_pool = rte_pktmbuf_pool_create(
      "bigc_dst_mbuf_pool", MBUF_POOL_SIZE, MEMPOOL_CACHE_SIZE, 0,
      DST_MBUF_DATA_ROOM + RTE_PKTMBUF_HEADROOM, rte_socket_id());
  struct rte_mempool *op_pool =
      rte_comp_op_pool_create("bigc_comp_op_pool", COMP_OP_POOL_SIZE,
                              MEMPOOL_CACHE_SIZE, 0, rte_socket_id());
  if (!src_pool || !dst_pool || !op_pool)
    rte_exit(EXIT_FAILURE, "mempool alloc failed\n");
  ...
  size_t comp_dpdk_raw_len = dpdk_compress_stateless(
      input_buf, INPUT_SIZE_BYTES, comp_dpdk, comp_cap, dpdk_raw_sizes,
      comp_zlib, comp_cap, dpdk_zlib_sizes, &comp_dpdk_zlib_len, src_pool,
      dst_pool, op_pool, dev_id, priv_xform_c);
  printf("[1] DPDK compress → zlib decompress: Compressed %zu bytes\n",
         comp_dpdk_raw_len);
  size_t decomp_zlib_len = 0;
  ret = zlib_decompress_concat(comp_zlib, comp_dpdk_zlib_len, decomp_zlib,
                               INPUT_SIZE_BYTES, &decomp_zlib_len);
  if (ret == 0 && decomp_zlib_len == INPUT_SIZE_BYTES &&
      memcmp(input_buf, decomp_zlib, INPUT_SIZE_BYTES) == 0)
    printf("[1] PASS: zlib解压后数据一致\n");
  else
    printf("[1] FAIL: zlib解压后数据不一致\n");

  // 2. zlib压缩→DPDK分片解压
  size_t comp_zlib_len = zlib_compress_chunks_raw(
      input_buf, INPUT_SIZE_BYTES, comp_zlib, comp_cap, zlib_raw_sizes);
  printf("[2] zlib compress → DPDK decompress: Compressed %zu bytes\n",
         comp_zlib_len);
  size_t decomp_dpdk_len = dpdk_decompress_stateless(
      comp_zlib, comp_zlib_len, zlib_raw_sizes, max_chunks, decomp_dpdk,
      INPUT_SIZE_BYTES, src_pool, dst_pool, op_pool, dev_id, priv_xform_d);
  if (decomp_dpdk_len == INPUT_SIZE_BYTES &&
      memcmp(input_buf, decomp_dpdk, INPUT_SIZE_BYTES) == 0)
    printf("[2] PASS: DPDK解压后数据一致\n");
  else
    printf("[2] FAIL: DPDK解压后数据不一致\n");

  // 3. DPDK分片压缩→DPDK分片解压
  size_t decomp_dpdk2_len = dpdk_decompress_stateless(
      comp_dpdk, comp_dpdk_raw_len, dpdk_raw_sizes, max_chunks, decomp_dpdk2,
      INPUT_SIZE_BYTES, src_pool, dst_pool, op_pool, dev_id, priv_xform_d);
  if (decomp_dpdk2_len == INPUT_SIZE_BYTES &&
      memcmp(input_buf, decomp_dpdk2, INPUT_SIZE_BYTES) == 0)
