...
struct VC_PACKED vc_asym_meta_v1 {
    ...
    uint16_t key_bits;     /* optional, keep 0 if unknown */
    uint16_t hash_algo;    /* for sign/verify or future use */
    uint16_t curve_id;     /* for ECDSA/ECDH, RSA keep 0 */
    uint16_t padding_algo; /* RAW / PKCS1 / PSS ... */
    uint16_t key_type;     /* VIRTIO_CRYPTO_AKCIPHER_KEY_TYPE_PUBLIC/PRIVATE */
};
