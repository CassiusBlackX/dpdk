...
struct virtio_comp_hw;

struct virtio_pci_ops {
	void (*read_dev_cfg)(struct virtio_comp_hw *hw, size_t offset,
			     void *dst, int len);
	void (*write_dev_cfg)(struct virtio_comp_hw *hw, size_t offset,
			      const void *src, int len);
	void (*reset)(struct virtio_comp_hw *hw);

	uint8_t (*get_status)(struct virtio_comp_hw *hw);
	void (*set_status)(struct virtio_comp_hw *hw, uint8_t status);

	uint64_t (*get_features)(struct virtio_comp_hw *hw);
	void (*set_features)(struct virtio_comp_hw *hw, uint64_t features);

	uint8_t (*get_isr)(struct virtio_comp_hw *hw);

	uint16_t (*set_config_irq)(struct virtio_comp_hw *hw, uint16_t vec);

	uint16_t (*set_queue_irq)(struct virtio_comp_hw *hw,
			struct virtqueue *vq, uint16_t vec);

	uint16_t (*get_queue_num)(struct virtio_comp_hw *hw,
			uint16_t queue_id);
	int (*setup_queue)(struct virtio_comp_hw *hw, struct virtqueue *vq);
	void (*del_queue)(struct virtio_comp_hw *hw, struct virtqueue *vq);
	void (*notify_queue)(struct virtio_comp_hw *hw, struct virtqueue *vq);
};
...
/*
 * While virtio_compress_hw is stored in shared memory, this structure stores
...
static inline int
vtpci_with_feature(struct virtio_comp_hw *hw, uint64_t bit)
{
	return (hw->guest_features & (1ULL << bit)) != 0;
}

static inline int
vtpci_with_packed_queue(struct virtio_comp_hw *hw)
{
